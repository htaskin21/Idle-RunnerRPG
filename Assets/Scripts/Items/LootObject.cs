using System.Collections.Generic;
using System.Threading;
using System.Threading.Tasks;
using Cysharp.Threading.Tasks;
using DG.Tweening;
using Enums;
using Managers;
using States;
using UnityEngine;

namespace Items
{
    public abstract class LootObject : MonoBehaviour
    {
        [SerializeField]
        private LootType lootType;

        protected double _lootAmount;

        [SerializeField]
        private float movementDuration;

        [SerializeField]
        protected List<GameObject> sprites;

        protected Vector3 _targetPosition;

        [SerializeField]
        private List<Vector3> _defaultPositions;

        public void SetInitialPosition(Transform enemyPosition, double amount)
        {
            _lootAmount = amount;

            var o = gameObject;
            var position = enemyPosition.transform.position;
            o.transform.position = new Vector3(position.x, 0, position.z);

            for (int i = 0; i < _defaultPositions.Count; i++)
            {
                sprites[i].transform.localPosition = _defaultPositions[i];
                sprites[i].transform.localScale = new Vector3(.5f, .5f, .5f);
            }

            o.SetActive(true);
        }

        public abstract UniTask MoveLoot(CancellationTokenSource cts);

        private void OnTriggerEnter2D(Collider2D col)
        {
            if (col.CompareTag("Player"))
            {
                var cts = new CancellationTokenSource();
                MoveLoot(cts).Forget();
            }
        }

        protected async Task<Sequence> TweenObjectToHud()
        {
            Sequence mySequence = null;

            foreach (var coinSprite in sprites)
            {
                mySequence = DOTween.Sequence();
                mySequence.Append(coinSprite.transform.DOMove(_targetPosition, movementDuration).SetEase(Ease.Linear))
                    .OnPlay(() =>
                        coinSprite.transform.DOScale(new Vector3(0, 0, 0), movementDuration).SetEase(Ease.InCirc));

                await UniTask.Delay(150);
            }

            return mySequence;
        }

        protected float GetTargetDifference()
        {
            var targetDifference = -0.5f;
            if (StageManager.Instance.HeroController.currentState.stateType == StateType.Run)
            {
                targetDifference = 0.5f;
            }

            return targetDifference;
        }

        protected abstract void SetTargetPosition();
    }
}